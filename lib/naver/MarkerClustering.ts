...
// @ts-nocheck
/**
 * 공식 네이버 marker-tools.js의 MarkerClustering 벤더링 (Apache-2.0 헤더 원본 유지).
 * 원본: https://github.com/navermaps/marker-tools.js/blob/master/marker-clustering/src/MarkerClustering.js
 *
...
type MarkerClusteringCtor = new (options: MarkerClusteringOptions) => MarkerClusteringInstance;

let cached: MarkerClusteringCtor | null = null;

export function getMarkerClusteringClass(): MarkerClusteringCtor {
...
naver.maps.Util.ClassExtend(MarkerClustering, naver.maps.OverlayView, {
	...
	onRemove: function() {
		// [벤더 패치 1건] 원본 오타 수정: _mapRelation → _mapRelations.
		// 원본은 onAdd에서 _mapRelations로 등록한 idle 리스너를 오타로 해제 못 해,
		// 파괴된 클러스터러의 _onIdle이 잔존 → null map으로 예외 → 이후 등록된
		// 새 클러스터러의 idle 재그리기까지 차단 (GL 지도에서 리빌드 시 마커 소실 실측)
...
Cluster.prototype = {
  ...
  // ---------- 원본 본문 끝 ----------
  cached = MarkerClustering as unknown as MarkerClusteringCtor;
  return cached;
}
