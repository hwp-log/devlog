...
export const theme = {
  ...
  light: {
    ...
    //
    // ⚠ 0590부터 **소비처 0**이다. 경고 문구가 면 배너(아래 warningSurface/Fg/Icon)로 바뀌며
    //   글씨·아이콘이 각자 토큰을 갖게 됐다. 값 판단(0589)은 유효하나 쓰는 곳이 없다 —
    //   제거는 별건(0590 범위 밖, 사용자 판단 대기).
    warning: '#d97706',
    // 0590: 경고 **면** 축 신설 — 0580의 "면·테두리 짝은 만들지 않는다"를 뒤집는다.
    //   0589로 글씨는 읽히게 됐지만 흰 배경에 주황 글씨만 떠 있어 튀어 보였다(실화면).
    //   업계 표준 경고 배너는 연한 면 + 진한 글씨 + 중간 톤 아이콘의 3종 조합이다 —
    //   면이 "여기부터 여기까지가 경고"라는 영역을 만들어 주기 때문에 글씨가 혼자 튀지 않는다.
    //   **테두리는 두지 않는다**(사용자 확정) — 면만으로 영역이 성립하고, 선이 붙으면
    //   danger 계열(테두리 있는 파괴 버튼)과 신호가 겹친다.
    //
    //   라이트 실측(배경 #ffffff 기준): 면 1.037:1(흰 배경에서 아주 옅게 뜸) /
    //   글씨 on 면 6.84:1(AA 여유) / 아이콘 on 면 2.07:1.
    //   아이콘 대비가 낮은 건 **알고 수용**한다 — 아이콘은 정보를 단독으로 지지 않고
    //   (문장이 같은 내용을 말하고 aria-hidden이다) 장식·시선 유도 역할이다.
    // ⚠ 0597부터 **소비처 0**이다(warning과 같은 처지). 담은 플랜 안내가 경고 면에서
    //   중립 블록(hairline/fill1 + primary 아이콘)으로 바뀌며 경고 축 4개가 전부 미사용이 됐다.
    //   값 판단(0590 실측)은 유효하나 쓰는 곳이 없다 — 제거는 별건(사용자 확정: 삭제 금지).
    warningSurface: '#fffbeb', // amber-50
    warningFg: '#92400e', // amber-800 — 면 위라 글씨는 더 진해도 된다(0589의 amber-600보다 진함)
    warningIcon: '#f59e0b', // amber-500 — 라이트·다크 같은 값(모드 무관 한 값인 유일한 경고 축)
  ...
  dark: {
    ...
    // ⚠ 0590부터 소비처 0 — 라이트 짝 주석 참조.
    warning: '#f59e0b',
    // 0590: 경고 면 축 다크 — 라이트 짝과 같은 이름·다른 값(라이트 주석에 도입 경위).
    //   면(#332a18): bg(#151718) 대비 **1.272:1로 다크 dangerSurface(#3a2626)와 같은 수치**다.
    //     면 계열이 얼마나 도드라질지를 기존 축과 맞췄다. card(#1d1f21) 위에서도 1.169:1로 뜬다.
    //     더 옅은 후보(#2a2115)는 card 대비 1.045라 카드 위에서 면이 사라졌다(실측 기각).
    //   글씨(#fcd34d, amber-300): 면 위 9.81:1. amber-200(#fde68a, 11.35:1)은 흰색에 가까워져
    //     "경고색"이 아니라 "그냥 밝은 글씨"가 되고, amber-400(#fbbf24)은 accent(#f0c040,
    //     별점 전용)와 너무 붙는다 — 0580이 다크 warning에서 amber-400을 피한 판단을 승계.
    //   아이콘(#f59e0b): 면 위 6.58:1. **라이트와 같은 값** — 중간 톤이라 양쪽 면에서 성립한다.
    // ⚠ 0597부터 소비처 0 — 라이트 짝 주석 참조.
    warningSurface: '#332a18',
    warningFg: '#fcd34d',
    warningIcon: '#f59e0b',
