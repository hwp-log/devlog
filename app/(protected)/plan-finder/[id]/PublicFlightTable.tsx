...
function LegRow({ seg, label }: { seg: FlightSegmentData; label: string }) {
  return (
    <div className="flex flex-col gap-[3px] sm:grid sm:grid-cols-[60px_1fr_max-content_max-content] sm:items-center sm:gap-5 sm:py-[15px] sm:border-b sm:border-[#f1f2f3]">
      <span className="text-[11px] sm:text-[13px] font-semibold text-muted shrink-0">{label}</span>

      <div className="flex items-center gap-2 flex-wrap min-w-0 text-base font-semibold text-fg">
        <span>{seg.origin}</span>
        <span className="sm:text-[13px] sm:font-normal sm:text-muted">{AIRPORT_NAME[seg.origin] ?? ''}</span>
        <span aria-hidden>→</span>
        <span>{seg.destination}</span>
        <span className="sm:text-[13px] sm:font-normal sm:text-muted">{AIRPORT_NAME[seg.destination] ?? ''}</span>
      </div>

      <span className="text-xs sm:text-[13px] text-muted shrink-0">
        {seg.durationLabel ?? ''} · 직항
        <span className="sm:hidden">
          {' '}· {seg.airline} {seg.flightNo}
        </span>
      </span>
      <span className="max-sm:hidden text-[13px] font-semibold text-fg2 shrink-0">
        {seg.airline} {seg.flightNo}
      </span>
    </div>
  );
